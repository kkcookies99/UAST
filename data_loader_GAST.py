...
def get_example(item):
    ...
    return [adj_matrix, fea_matrix, label]
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        adj_matrix = self.examples[index][0]  # [1,5,3,6,...]
        fea_matrix = self.examples[index][1]
        label = self.examples[index][2]
        return adj_matrix, fea_matrix, label
        # return torch.tensor(torch.tensor(self.examples[item].label))


def collate_fn(batch):
    adj_matrix, fea_matrix, label = list(zip(*batch))
...
def get_graph_dataloader(train="train"):
    ...
    if train == "train":
        ...
        data_loader = DataLoader(test_data, batch_size=config.batch_size, shuffle=True, collate_fn=collate_fn,
                                 num_workers=0)
...
if __name__ == '__main__':
    data_loader = tqdm(get_graph_dataloader(train="train"))
